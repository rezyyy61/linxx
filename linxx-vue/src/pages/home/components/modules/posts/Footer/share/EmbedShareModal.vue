<template>
  <Teleport to="body">
    <transition name="scale-fade">
      <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4" @click.self="close">
        <div class="w-full max-w-[92vw] sm:max-w-lg md:max-w-xl lg:max-w-2xl rounded-3xl shadow-xl ring-1 ring-white/10 dark:ring-white/20 overflow-hidden flex flex-col max-h-[92vh] bg-white/90 dark:bg-neutral-900/95 backdrop-blur-2xl">
          <!-- Header -->
          <header class="flex items-center justify-between px-6 py-4 bg-gradient-to-br from-white/70 to-white/30 dark:from-neutral-800/60 dark:to-neutral-800/30 backdrop-blur-sm border-b border-white/20 dark:border-white/10">
            <div class="flex items-center gap-3 min-w-0">
              <UserAvatar :src="avatarSrc" :fallback="userName" :color="avatarColor" size="md" />
              <div class="flex flex-col min-w-0">
                <span class="font-semibold text-gray-800 dark:text-gray-100 truncate">{{ userName }}</span>
                <span class="text-xs text-gray-500 dark:text-gray-400">Share to your feed</span>
              </div>
            </div>
            <button class="text-gray-400 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 text-xl leading-none transition-colors" @click="close" aria-label="Close">✕</button>
          </header>

          <!-- Body -->
          <section class="p-8 space-y-5 overflow-y-auto">
            <div class="rounded-2xl bg-gradient-to-br from-blue-100/50 to-indigo-100/30 dark:from-blue-500/20 dark:to-indigo-500/10 p-1">
              <div ref="editorBoxRef" class="share-compact-editor rounded-[14px] bg-white/80 dark:bg-neutral-800/80">
                <TiptapEditor v-model="caption" />
              </div>
              <div class="mt-1 px-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 select-none">
                <span class="truncate">{{ captionPlain ? ' ' : $t(baseKey + '.hint') }}</span>
              </div>
            </div>

            <div class="border border-white/30 dark:border-white/15 rounded-2xl overflow-hidden max-w-max mx-auto shadow-sm">
              <!-- پیش‌نمایش: پست یا رویداد -->
              <PostCard v-if="!isEvent" :post="sharedPost" :embed="true" />
              <EventEmbedCard v-else :event="sharedPost" />
            </div>
          </section>

          <!-- Footer -->
          <footer class="px-6 py-4 border-t border-white/20 dark:border-white/10 bg-gradient-to-t from-white/60 to-white/20 dark:from-neutral-800/60 dark:to-neutral-800/30 backdrop-blur-sm flex gap-3 justify-end">
            <button class="px-5 py-2 rounded-full border border-gray-300 dark:border-gray-600 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-neutral-800 transition" :disabled="submitting" @click="close">
              {{ $t(baseKey + '.cancel') }}
            </button>
            <button class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:from-blue-400 disabled:to-indigo-400 text-white font-semibold rounded-full px-8 py-2 shadow-lg shadow-blue-600/30 transition-all active:scale-95 flex items-center justify-center min-w-[110px]"
                    :disabled="submitting || !canSubmit || (captionLimit && captionCount > captionLimit)"
                    @click="submit">
              <span v-if="!submitting">{{ $t(baseKey + '.action') }}</span>
              <svg v-else class="animate-spin h-5 w-5 text-white" viewBox="0 0 24 24" fill="none">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                <path class="opacity-75" fill="currentColor" d="M4 12a 8 8 0 0 1 8-8v4a4 4 0 0 0-4 4H4z" />
              </svg>
            </button>
          </footer>
        </div>
      </div>
    </transition>
  </Teleport>
</template>

<script setup>
import { ref, computed, watch, nextTick } from 'vue'
import PostCard from '@/pages/home/components/modules/posts/PostCard.vue'
import EventEmbedCard from '@/pages/home/components/modules/events/EventEmbedCard.vue'
import UserAvatar from '@/pages/auth/UserAvatar.vue'
import TiptapEditor from '@/pages/admin/sections/TiptapEditor.vue'
import { useAuthStore } from '@/stores/auth'

const baseKey = 'post.shares.key'

const props = defineProps({
  visible: Boolean,
  sharedPost: { type: Object, default: null },
  sharedType: { type: String, default: '' },
  captionLimit: { type: Number, default: 280 }
})

const emit = defineEmits(['close', 'submitted'])

const authStore = useAuthStore()
const user = computed(() => authStore.user ?? {})
const userName = computed(() => user.value.name ?? '')
const avatarColor = computed(() => user.value.avatar_color ?? null)
const avatarSrc = computed(() => user.value.logo_url ?? null)

const caption = ref('')
const submitting = ref(false)
const editorBoxRef = ref(null)

const captionPlain = computed(() =>
    (caption.value || '').replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim()
)
const captionCount = computed(() => captionPlain.value.length)

const isEvent = computed(() => {
  if (props.sharedType) return props.sharedType === 'event'
  const s = props.sharedPost || {}
  // حدس ساده: event معمولاً title دارد و یکی از starts_at/ends_at
  return ('title' in s) && ('starts_at' in s || 'ends_at' in s)
})
const canSubmit = computed(() => !!props.sharedPost?.id)

watch(
    () => props.visible,
    async v => {
      if (v) {
        if (!authStore.user) await authStore.fetchUser()
        await nextTick()
        const el = editorBoxRef.value?.querySelector?.('.ql-editor')
        if (el) el.focus()
      } else {
        caption.value = ''
      }
    }
)

function close() {
  caption.value = ''
  emit('close')
}

async function submit() {
  if (submitting.value || !canSubmit.value) return
  if (props.captionLimit && captionCount.value > props.captionLimit) return
  submitting.value = true
  emit('submitted', {
    text: caption.value,
    shared_post_id: props.sharedPost.id
  })
  caption.value = ''
  submitting.value = false
  close()
}
</script>

<style scoped>
.scale-fade-enter-from { opacity: 0; transform: scale(0.92); }
.scale-fade-enter-active { transition: all 0.25s ease; }
.scale-fade-leave-active { transition: all 0.2s ease; opacity: 0; transform: scale(0.92); }

:deep(.share-compact-editor .ql-toolbar){ padding:6px 8px; }
:deep(.share-compact-editor .ql-toolbar .ql-formats){ margin-right:4px; }
:deep(.share-compact-editor .ql-toolbar button){ width:26px; height:26px; }

:deep(.share-compact-editor .ql-container){ min-height:120px; }
:deep(.share-compact-editor .ql-editor){ min-height:100px; padding:10px 12px; font-size:.95rem; line-height:1.5; }
</style>
